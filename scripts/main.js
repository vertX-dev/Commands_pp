...
system.beforeEvents.startup.subscribe((init) => {
        ...
        mandatoryParameters: [
            {
                ...
                name: "Item Name (e.g. '§6Epic Sword')",
            },
        ],
    };
    ...
    const createPathCommand = {
        ...
        optionalParameters: [
            ...
            {
                type: CustomCommandParamType.String,
                name: "Path block (or 'MAINHAND' or 'MULTIBLOCK')",
            },
        ],
    };
    ...
    init.customCommandRegistry.registerCommand(showLoreNumbersCommand, showLoreNumbersFunction);
    init.customCommandRegistry.registerCommand(replaceLoreLineCommand, replaceLoreLineFunction);
...
function systemRunIntervalFunction(origin, runType, query, delay, repeat) {
    let count = 0;
...
function systemRunTimeoutFunction(origin, runType, query, delay) {
    system.runTimeout(() => {
        systemRunFunction(origin, runType, query);
    }, delay);
}
    ...
    system.run(() => {
        for (const entity of entities) {
            try {
                ...
                const newItem = item.clone();
                const oldLore = item.getLore();
                const newLore = [...oldLore, hideString(command.replace(/'/g, '"'), COMMAND_MARKER)];
                newItem.setLore(newLore);
                equippable.setEquipment(EquipmentSlot.Mainhand, newItem);
            } catch (e) {
                    ...
                    if (distanceSquared <= radiusSquared) {
                        ...
                        try {
                            ...
                            if (!shouldReplaceAll) {
                                ...
                                const shouldReplace = replaceList.some((replaceBlock) => currentBlockId.includes(replaceBlock) || replaceBlock === currentBlockId);

                                if (!shouldReplace) continue;
                            }

                            // Choose block to place
                            const blockToPlace = isMultiBlock ? pathBlocks[Math.floor(Math.random() * pathBlocks.length)] : pathBlocks[0];
    ...
    if (inventory && inventory.container) {
        ...
        for (let slot = 0; slot < container.size; slot++) {
            try {
                ...
                const matches = itemId.includes(query) || displayNameLower.includes(query) || itemId === query || displayNameLower === query;
...
function processBatchFill(fillState) {
    const blocksToProcess = Math.min(BLOCK_FILLING_CONFIG.BLOCKS_PER_TICK, fillState.positions.length - fillState.currentIndex);
    ...
    if (fillState.placedBlocks % BLOCK_FILLING_CONFIG.PROGRESS_UPDATE_INTERVAL === 0 || fillState.currentIndex >= fillState.positions.length) {
        ...
        fillState.entity.sendMessage(`§7Progress: ${progress}% (${fillState.placedBlocks}/${fillState.totalBlocks}) - ${elapsed}s elapsed`);
    }
...
function processConnectedPath(pathState) {
    ...
    if (pathState.currentStep % 20 === 0 || pathState.currentStep >= pathState.totalSteps) {
        ...
        pathState.player.sendMessage(`§7Step ${pathState.currentStep}/${pathState.totalSteps} (${segmentProgress}%) - ${pathState.totalPlaced} blocks placed - ${elapsed}s`);
    }
}
                ...
                if (shouldPlaceAtLocation(centerX, centerY, centerZ, x, y, z, pathState.radius, pathState.mode)) {
                    ...
                    if (!pathState.shouldReplaceAll && pathState.replaceList.length > 0) {
                        try {
                            ...
                            const shouldReplace = pathState.replaceList.some((replaceBlock) => currentBlockId.includes(replaceBlock));
...
function processSegmentCalculation(pathState) {
    ...
    const segmentPositions = calculatePathSegmentPositionsBatched(from, to, pathState.radius, pathState.mode, pathState.dimension, pathState.shouldReplaceAll, pathState.replaceList);
            ...
            for (let y = centerY - Math.ceil(scanRadius); y <= centerY + Math.ceil(scanRadius) && positions.length < maxPositions; y++) {
                for (let z = centerZ - Math.ceil(scanRadius); z <= centerZ + Math.ceil(scanRadius) && positions.length < maxPositions; z++) {
                    if (shouldPlaceAtLocation(centerX, centerY, centerZ, x, y, z, radius, mode)) {
                        ...
                        const isDuplicate = recentPositions.some((pos) => pos.x === location.x && pos.y === location.y && pos.z === location.z);
                        ...
                        if (!isDuplicate) {
                            ...
                            if (!shouldReplaceAll && replaceList.length > 0) {
                                try {
                                    ...
                                    const shouldReplace = replaceList.some((replaceBlock) => currentBlockId.includes(replaceBlock));
...
function processTextBuilding(buildState) {
    ...
    if (buildState.currentRow >= pattern.length) {
        ...
        buildState.player.sendMessage(`§7Character ${buildState.currentChar}/${buildState.totalChars} (${progress}%) - ${buildState.blocksPlaced} blocks - ${elapsed}s`);
    }
}
...
function processLargeFillBatch(fillState) {
    ...
    if (fillState.processedBlocks % 10000 === 0 || fillState.isComplete) {
        ...
        fillState.player.sendMessage(`§7Progress: ${progress}% (${fillState.processedBlocks.toLocaleString()}/${fillState.totalVolume.toLocaleString()}) - ${rate}/s - ${elapsed}s`);
    }
...
function processOutlineMode(fillState, location) {
    const isEdge =
        location.x === fillState.minX ||
        location.x === fillState.maxX ||
        location.y === fillState.minY ||
        location.y === fillState.maxY ||
        location.z === fillState.minZ ||
        location.z === fillState.maxZ;
...
function processFigureBatch(figureState) {
    ...
    if (figureState.currentIndex % 1000 === 0 || figureState.currentIndex >= figureState.positions.length) {
        ...
        figureState.player.sendMessage(`§7Progress: ${progress}% (${figureState.currentIndex}/${figureState.positions.length}) - ${elapsed}s`);
    }
...
function processSurfaceBatch(surfaceState) {
    ...
    if (surfaceState.positionsProcessed % 500 === 0 || surfaceState.currentIndex >= surfaceState.positions.length) {
        ...
        surfaceState.player.sendMessage(`§7Progress: ${progress}% (${surfaceState.currentIndex}/${surfaceState.positions.length}) - ${elapsed}s`);
    }
...
function veinToolInfoFunction(origin) {
    system.run(() => {
        try {
            ...
            const toolState = VEIN_TOOL_STATES.get(player.id) || {
                corners: [],
                breakCount: 0,
            };
...
function placeVeinBlocks(veinState) {
    ...
    if (veinState.currentIndex % 100 === 0 || veinState.currentIndex >= veinState.allVeinPositions.length) {
        ...
        veinState.player.sendMessage(`§7Progress: ${progress}% (${veinState.currentIndex}/${veinState.allVeinPositions.length}) - ${veinState.blocksPlaced} blocks - ${elapsed}s`);
    }
...
function findNearestPoints(targetPoint, allPoints, maxCount) {
    const distances = allPoints
        ...
        .map((p) => ({
            point: p,
            distance: Math.sqrt(Math.pow(p.x - targetPoint.x, 2) + Math.pow(p.y - targetPoint.y, 2) + Math.pow(p.z - targetPoint.z, 2)),
        }))
...
function applyMaxEnchantments(player, variant) {
    try {
        ...
        return {
            success: false,
            reason: `Error applying enchantments: ${e.message || e}`,
        };
    }
}
    ...
    for (const cmd of COMMANDS) {
        ...
        if (score > 0) {
            results.push({
                ...cmd,
                score: score,
                matchedTerms: searchTerms.filter((term) => commandText.includes(term) || descriptionText.includes(term) || keywords.some((kw) => kw.includes(term) || term.includes(kw))),
            });
        }
    }
        ...
        if (includeEntities) {
            ...
            for (const entity of entities) {
                if (
                    ...
                    entity.location.y <= to.y &&
                    entity.location.z >= from.z &&
                    entity.location.z <= to.z
                ) {
                    try {
                        entity.remove();
                    } catch (e) {
                ...
                try {
                    ...
                    if (score !== undefined && score > 0) {
                        ...
                        if (newScore <= 0) {
                            ...
                            if (newScore % 30 === 0 && newScore <= 120) {
                                // Every 30s for last 2 minutes
...
// Replace lore at specific index
function replaceLoreLineFunction(origin, lineIndex, newLine, targets = [origin.sourceEntity]) {
    system.run(() => {
        try {
            let modifiedCount = 0;
            ...
            for (const entity of targets) {
                ...
                // Create new lore array with replaced line
                const newLore = [...currentLore];
                const oldLine = newLore[lineIndex];
                newLore[lineIndex] = newLine;
...
// Replace lore line by finding text match
function replaceLoreLineFindFunction(origin, findText, newLine, targets = [origin.sourceEntity]) {
    system.run(() => {
        try {
            let modifiedCount = 0;
            ...
            for (const entity of targets) {
                ...
                for (let i = 0; i < currentLore.length; i++) {
                    ...
                    // Check if line contains the search text (case insensitive)
                    if (line.toLowerCase().includes(findText.toLowerCase())) {
                        matchingLines.push({
                            index: i,
                            text: line,
                        });
                ...
                const targetMatch = matchingLines[0];
                const newLore = [...currentLore];
                newLore[targetMatch.index] = newLine;
...
function showLoreNumbersFunction(origin, targets = [origin.sourceEntity]) {
    system.run(() => {
        try {
            for (const entity of targets) {
                if (!entity || entity.typeId !== "minecraft:player") {
